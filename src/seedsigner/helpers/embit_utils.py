...
def _collect_tap_leaves(tree, _depth: int = 0) -> list:
    """
    Same recursion shape and depth guard as _count_tap_leaves above, but
    collects the actual TapLeaf objects in the tree's left-to-right order
    instead of just counting them.

    This order matches embit's own _tweak_helper leaf enumeration
    (left-subtree leaves, then right-subtree leaves) and, for a
    DynastyTrust-compiled tr_multileaf tree, matches the compiler's own
    intended leaf order (founders_now, recovery, inheritance,
    second_inheritance, or a custom leaf-list's declared order) -- see
    policy_compiler.rs's nest_leaves()/TaprootBuilder depth schedule,
    which builds exactly the right-leaning {L1,{L2,{L3,L4}}} shape this
    walks depth-first, left branch first.
    """
    if tree is None or tree.tree is None:
        return []
    if _depth > _MAX_TAP_TREE_DEPTH:
    ...
    if isinstance(tree.tree, tuple):
        ...
        return _collect_tap_leaves(left, _depth + 1) + _collect_tap_leaves(right, _depth + 1)
    return [tree.tree]



def get_taproot_leaf_summaries(descriptor: Descriptor) -> list:
    """
    Per-leaf quorum/timelock facts for EVERY leaf in a taproot descriptor's
    tap tree, in the tree's own left-to-right leaf order (index 0 = leaf
    1). Returns raw data, not formatted strings -- same convention as
    get_taproot_policy_summary and PSBTParser.get_signing_leaf_summary.

    This is the whole-tree counterpart to
    PSBTParser.get_signing_leaf_summary, which only ever describes the ONE
    leaf a PSBT is signing: the registration confirmation screen needs to
    show the WHOLE policy, before any PSBT exists, so a person can verify
    every path their vault actually has -- not just the one path they
    happen to be about to spend from, and not the illegible raw nested-
    parens policy string. Reuses PSBTParser._parse_leaf_quorum/
    _decode_timelock directly -- the same, already-hardened AST-walking
    logic already proven correct for a single leaf at signing time --
    rather than a second, independently-maintained copy of that parsing.

    No PSBT and no derived key is needed: quorum counts and timelock
    values are literal/structural facts about the policy, not dependent
    on which child index a ranged key was derived to, so this walks
    descriptor.taptree directly.

    Returns a list of dicts, one per leaf, in tree order:
        {
            "quorum_k": int | None,
            "quorum_n": int | None,
    ...
    An unrecognized leaf shape (not one of the small set of shapes a
    tr_multileaf-style policy actually produces -- see
    PSBTParser._parse_leaf_quorum's own docstring) reports all-None
    fields for that leaf rather than guessing, same "can't prove it, say
    nothing" rule every other summary in this file follows.
    """
    if not descriptor.is_taproot:
    ...
    try:
        leaves = _collect_tap_leaves(descriptor.taptree)
    except ValueError:
        logger.warning("Tap tree exceeds max depth; no leaf summaries available", exc_info=True)
        return []

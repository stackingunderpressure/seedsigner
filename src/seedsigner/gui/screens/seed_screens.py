...
def _format_leaf_summary_line(index: int, leaf: dict) -> str:
    """
    One compact line per leaf: "Leaf N: k of n, <when>" -- the plain-
    English counterpart to the raw nested-parens policy string this
    screen exists to replace. Mirrors PSBTSpendPathScreen's own
    quorum/timelock phrasing exactly (same _() strings, same timelock_kind
    branches) so a person sees identical wording whether they're
    reviewing the whole policy at registration or one leaf at signing
    time.
    """
    from seedsigner.gui.screens.psbt_screens import _format_relative_duration
    import datetime

    if leaf["quorum_k"] is None or leaf["quorum_n"] is None:
        # An unrecognized leaf shape -- PSBTParser._parse_leaf_quorum only
        # knows the small set of shapes a tr_multileaf-style policy
        # actually produces; anything else reports all-None rather than
        # guessing. Say so honestly instead of a blank or wrong line.
        # TRANSLATOR_NOTE: e.g. "Leaf 3: custom script" -- this leaf's spending rule couldn't be summarized
        return _("Leaf {index}: custom script").format(index=index)

    # TRANSLATOR_NOTE: e.g. "2 of 3" -- how many signatures this leaf requires
    quorum_text = _("{k} of {n}").format(k=leaf["quorum_k"], n=leaf["quorum_n"])

    timelock_kind = leaf["timelock_kind"]
    timelock_value = leaf["timelock_value"]
    if timelock_kind is None:
        # TRANSLATOR_NOTE: This leaf has no timelock -- it can be spent right away
        when_text = _("spendable now")
    elif timelock_kind == "after_height":
        # TRANSLATOR_NOTE: e.g. "after block 840000" -- an absolute-height timelock (CLTV)
        when_text = _("after block {n}").format(n=timelock_value)
    elif timelock_kind == "after_time":
        # TRANSLATOR_NOTE: e.g. "after 2026-01-01" -- a calendar-date timelock (CLTV encoded as a Unix timestamp)
        timelock_date = datetime.datetime.fromtimestamp(timelock_value, tz=datetime.timezone.utc).strftime("%Y-%m-%d")
        when_text = _("after {date}").format(date=timelock_date)
    elif timelock_kind == "older_blocks":
        # TRANSLATOR_NOTE: e.g. "after 144 blocks of age" -- a relative-age timelock (CSV), block-based
        when_text = _("after {n} blocks of age").format(n=timelock_value)
    else:  # "older_time"
        # TRANSLATOR_NOTE: e.g. "after ~30 days of age" -- a relative-age timelock (CSV), time-based
        when_text = _("after {duration} of age").format(duration=_format_relative_duration(timelock_value))

    # TRANSLATOR_NOTE: One line of a taproot policy's leaf-by-leaf breakdown, e.g. "Leaf 2: 2 of 3, after 25920 blocks of age"
    return _("Leaf {index}: {quorum}, {when}").format(index=index, quorum=quorum_text, when=when_text)



@dataclass
class MultisigWalletPolicyDetailScreen(ButtonListScreen):
    """
    Paginated, plain-English breakdown of EVERY leaf in a registered
    taproot descriptor's policy -- the whole-tree counterpart to
    PSBTSpendPathScreen, which only ever shows the one leaf being signed.
    Reached only from MultisigWalletDescriptorScreen's "Policy details"
    button. Paginates the same way SeedSignMessageConfirmMessageScreen
    does (reflow_text_into_pages against the real canvas size) since a
    leaf-list vault's decay ladder can produce more leaves than fit on
    one screen -- silently truncating them would defeat the entire point
    of this screen.
    """
    leaf_summaries: list = None
    page_num: int = 0
    ...
    def __post_init__(self):
        ...
        lines = [
            _format_leaf_summary_line(i + 1, leaf)
            for i, leaf in enumerate(self.leaf_summaries or [])
        ]
        ...
        # Always at least one page, even for an empty/unwalkable policy,
        # so page_num 0 is always valid and the View's pagination math
        # (page_num < num_pages - 1) never divides by a zero-length list.
        if not pages:
            pages = [""]
        ...
        if self.num_pages > 1:
            # TRANSLATOR_NOTE: e.g. "Policy Details (2/3)" -- current page of a multi-page policy breakdown
            self.title = _("Policy Details ({page}/{total})").format(page=self.page_num + 1, total=self.num_pages)
        else:
            self.title = _("Policy Details")
        self.is_bottom_list = True
        self.is_button_text_centered = True
        is_last_page = self.page_num == self.num_pages - 1
        self.button_data = [ButtonOption("Continue" if is_last_page else "Next")]
